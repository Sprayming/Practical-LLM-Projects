...
def read_file(file_path, sheet_name=0):
    ...
    if ext == ".csv":
        ...
        try: return pd.read_excel(file_path, sheet_name=sheet_name, engine="openpyxl")
            ...
            except: raise ValueError("无法解析该 Excel 文件，可能已损坏或加密")
...
def parse_as_orders(df):
    ...
    if "order_id" not in df.columns or "driver_id" not in df.columns:
        return None, ["无法识别为订单数据（缺少 order_id/driver_id）"]
    orders, errors = [], []
...
def load_any_file(file_path):
    ...
    for sheet in sheets:
        try:
            ...
            warnings.append(f"[{sheet}] 读取失败: {str(e)[:60]}")
            continue
    ...
    if datasets:
        ...
        return {
            ...
            "warnings": warnings[:5],
            "preview": list(datasets.values())[0]["rows"][:3] if datasets else [],
        }
