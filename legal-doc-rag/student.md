...
- 为什么本地 **BGE-M3** 而非线上 embedding：零成本 / 零泄露 / 面试加分"检索不依赖外部服务"；但切换后需清空 `chroma_db` 重新向量化。
  ...
  - **BGE-M3 是纯文本模型**：本身不处理图片。扫描件 PDF（无文字层、整页是图）PyMuPDF 抽不到文字，必须先用 **PaddleOCR** 把图片转文字再向量化——这不是"BGE-M3 多模态不行"，而是缺 OCR 引擎。OCR 链路已打通（见 README「OCR 引擎」章节 + 启动 bat 的 `.ocr_venv`）。
