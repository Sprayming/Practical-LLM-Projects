...
from app.models import DocumentMetaModel
...
class VectorStore:
    ...
    def search(
        ...
        for i in range(len(results["ids"][0])):
             # 修复2：安全获取 distance，防止索引越界
            # score=1.0-(results["distances"][0][i]if results["distances"] else 0.0) #距离越小，相似度越高
            dist = results["distances"][0][i] if results["distances"] and len(results["distances"][0]) > i else 0.0
            score = 1.0 - dist
            # 修复2：安全获取 metadata，防止索引越界和 None 值
    ...
    def list_documents(self) -> list[DocumentMetaModel]:
        """列出所有文档的元信息。"""
        ...
        return [DocumentMetaModel(**v) for v in doc_map.values()]
